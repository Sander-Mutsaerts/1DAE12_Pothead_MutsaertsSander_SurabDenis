...
#pragma region CollisionFunctionality
	...
	float GetDistance(const float x1, const float y1, const float x2, const float y2)
	{
		...
		float result{ sqrt(powf((maxX - minX), 2.f) +
							powf((maxY - minY), 2.f)) };
		return result;
	}
	...
	bool IsPointInRect(const Point2f point, const Rectf rect)
	{
		...
		{
			if (point.y < (rect.bottom + rect.height))
			{
				withinRangeY = true;
			}
		}
		if (withinRangeX && withinRangeY) return true;
		else return false;
	}

	bool IsOverlapping(const Circlef circle1, const Circlef circle2)
	{
		const float collisionDistance{ circle1.radius + circle2.radius };
		if (GetDistance(circle1.center, circle2.center) < collisionDistance) return true;
		else return false;
	}

	bool IsOverlapping(const Rectf rect1, const Rectf rect2)
	{
		const Point2f	rect1Center{ rect1.left + rect1.width / 2.f, rect1.bottom + rect1.height / 2.f },
						rect2Center{ rect2.left + rect2.width / 2.f, rect2.bottom + rect2.height / 2.f };
		const float rectMaxX{ (rect1.width / 2.f) + (rect2.width / 2.f) },
					rectMaxY{ (rect1.height / 2.f) + (rect2.height / 2.f) };

		if ((std::max(rect1Center.x, rect2Center.x) - std::min(rect1Center.x, rect2Center.x) ) < rectMaxX)
		{
			if ((std::max(rect1Center.y, rect2Center.y) - std::min(rect1Center.y, rect2Center.y)) < rectMaxY)
			{
				return true;
			}
		}
...
#pragma endregion CollisionFunctionality
	...
	std::string ToString(const Vector2f vector)
	{
		...
		strVector = "[" + std::to_string(vector.x) + ", " + std::to_string(vector.y) + "]";
		return strVector;
	}
	...
	{
		const float result{ (vector1.x * vector2.y) - (vector1.y * vector2.y) };
		return result;
	}

	float Length(const Vector2f vector)
	{
		const float result{ sqrtf(powf(vector.x, 2.f) + powf(vector.y, 2.f)) };
		return result;
	}

	Vector2f Scale(const float scalar, const Vector2f vector)
	{
		const Vector2f result{ scalar * vector.x, scalar * vector.y };
		return result;
	}
	...
	{
		const float result{ (DotProduct(vector1, vector2)) / (Length(vector1) + Length(vector2)) };
		return result;
	}
	...
	bool AreEqual(const Vector2f vector1, const Vector2f vector2)
	{
		...
		if (abs((vector1.x - vector2.x)) <= threshold)
		{
			if (abs((vector1.x - vector2.x)) <= threshold)
			{
				return true;
	...
	{
		const float scalar{ DotProduct(projector, normalized) };
